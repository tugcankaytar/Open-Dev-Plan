...
**Şu an çalışıyor:** SQLite şema + migration'lar · yerel LLM sağlayıcı soyutlaması (Ollama, şemaya kısıtlanmış JSON çıktı, streaming chat) · dayanıklı/GPU-koordineli iş kuyruğu · toplantı → yapılandırılmış çıkarım hattı (aksiyon maddesi, karar, özet) — her sonuç bir *proposal* olarak düşer, hiçbiri otomatik olarak bir domain tablosuna yazılmaz · deterministik (LLM'siz) toplantı planlayıcı ve çakışma tespiti · RRULE destekli `.ics` dışa/içe aktarım · tam HTTP API + SSE streaming (iş ilerlemesi ve sohbet) · çıkarım modelini tahminle değil veriyle seçen bir eval harness · kendi onayladığınız/düzelttiğiniz proposal'ları okuyan bir fine-tuning veri seti ihraç script'i · yeniden tasarlanmış bir React web arayüzü — gerçek grafik/istatistikli panel, projeler, toplantılar (transkript yapıştırma + canlı çıkarım + proposal inceleme), görev Kanban'ı, doğal dilden planlama asistanı, takvim içe/dışa aktarım, çalışma anında model seçici, ve kendi proje/görev/toplantı/karar verinle konuşan kalıcı bir sohbet asistanı — API ile aynı süreçten serviste.

**Henüz kurulmadı:** Gantt/kritik yol görünümü · canlı toplantı kaydı + Whisper deşifresi (elle transkript yapıştırma bir yer tutucu olarak bağlı — toplantı detay sayfasına bakın) · hibrit tam metin/anlamsal arama (sohbet asistanı şu an alaka düzeyine göre değil, güncel/açık veriyi doğrudan okuyarak çalışıyor) · günlük brief/rapor · kurulum sihirbazı · Kanban panosunda sürükle-bırak (şimdilik durum değişikliği açılır menüyle). Sürüm sürüm neyin geldiği için `CHANGELOG.md`'ye bakın.
