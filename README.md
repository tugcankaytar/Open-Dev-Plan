...
**Working now:** SQLite schema + migrations · local-LLM provider abstraction (Ollama, schema-constrained JSON output, streaming chat with tool-calling) · durable/GPU-coordinated job queue · meeting → structured extraction pipeline (action items, decisions, summary), with every result landing as a human-reviewed *proposal*, never written to a domain table automatically · deterministic (non-LLM) meeting scheduler and conflict detection · `.ics` export/import with RRULE, tucked into popups on a real calendar month view · full HTTP API + SSE streaming (job progress, chat, and a live-update event stream so the whole UI refreshes instantly with no manual reload) · an eval harness that picks the extraction model from data, not assumption · a fine-tuning dataset export script that reads your own approved/corrected proposals · customers linked to projects (many-to-many) and to meetings, and detailed tasks (priority, start/due dates, tags, a checklist, dependencies) created/edited through a full form · a draggable Kanban board with a list-view toggle · a redesigned React web UI — dashboard with real charts/stats, projects, meetings (transcript paste + live extraction + proposal review), a runtime model picker, and a persistent chat assistant with full tool-calling parity to the UI (it can create, update, and delete anything you can, suggest meeting slots, and manage task dependencies — with an explicit confirm-before-delete gate) grounded in your own projects/tasks/meetings/decisions — served by the same process as the API.

**Not yet built:** a Gantt/critical-path view · live meeting recording + Whisper transcription (manual transcript paste is wired up as a stand-in — see the meeting detail page) · hybrid full-text/semantic search (the chat assistant currently reads recent/open data directly, not a relevance search) · daily briefing/reports · an installer/setup wizard. See `CHANGELOG.md` for what's landed release by release.
