...
**Working now:** SQLite schema + migrations · local-LLM provider abstraction (Ollama, schema-constrained JSON output, streaming chat) · durable/GPU-coordinated job queue · meeting → structured extraction pipeline (action items, decisions, summary), with every result landing as a human-reviewed *proposal*, never written to a domain table automatically · deterministic (non-LLM) meeting scheduler and conflict detection · `.ics` export/import with RRULE · full HTTP API + SSE streaming (job progress and chat) · an eval harness that picks the extraction model from data, not assumption · a fine-tuning dataset export script that reads your own approved/corrected proposals · a redesigned React web UI — dashboard with real charts/stats, projects, meetings (transcript paste + live extraction + proposal review), a task Kanban, a natural-language scheduling assistant, calendar import/export, a runtime model picker, and a persistent chat assistant that answers questions grounded in your own projects/tasks/meetings/decisions — served by the same process as the API.

**Not yet built:** a Gantt/critical-path view · live meeting recording + Whisper transcription (manual transcript paste is wired up as a stand-in — see the meeting detail page) · hybrid full-text/semantic search (the chat assistant currently reads recent/open data directly, not a relevance search) · daily briefing/reports · an installer/setup wizard · drag-and-drop on the Kanban board (status changes via dropdown for now). See `CHANGELOG.md` for what's landed release by release.
